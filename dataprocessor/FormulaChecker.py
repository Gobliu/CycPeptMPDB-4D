"""Check that a PDB structure's molecular formula matches its reference SMILES.

Counts elements (hydrogens included) in the PDB and in the RDKit-parsed SMILES, then
compares them — catching structures that don't correspond to the intended molecule.
A geometry-only sanity gate, analogous to DLChecker for chirality.
"""
from collections import defaultdict
from rdkit import Chem


def formula_matches(pdb, smiles):
    ...
    if not match_flag:
        print('!!!Not match', 'From pdb', element_dict_pdb, 'From smiles', element_dict_smiles)
